...
- **Any CLI agent.** Built-in runtimes for `claude-code`, `sapling`, `codex`, `pi`, and `pi-chat`. New agents land via a declarative `[[agents]]` stanza in `burrow.toml` — zero core code changes.
