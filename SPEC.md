...
- **`pi`** — Spawn-per-turn. Default argv is `pi --mode rpc --session-dir .pi/sessions --no-extensions --offline --provider anthropic --model <pinned>` (pinned to a Claude model that matches the parser's golden RPC fixtures). `frontmatter.provider`/`model` override provider/model; the allowlisted `frontmatter.pi` option bag can add batch-safe Pi flags (`extensions`, `approve`, `tools`, `excludeTools`, `noTools`, `noBuiltinTools`, `extension`, `skill`, `promptTemplate`, `theme`) without changing the default argv shape. `tools` and `excludeTools` are generic Pi tool-name filters and may name built-in tools or tools registered by any loaded extension. When `frontmatter.pi.extensions = true`, `--no-extensions` is elided and the runtime auto-declines `extension_ui_request` with `cancelled:true` so interactive extension UI cannot hang a batch run. `--offline` suppresses pi's startup network operations (telemetry / update checks) that otherwise block the RPC read loop for 2+ minutes inside bwrap before the first event lands (burrow-029d). One `{"type":"prompt","message":"<prompt + steering prefix>"}` line on stdin per run; events stream on stdout. `supportsResume = true`: `prepareWorkspace` creates the per-burrow session dir, `extractMetadata` reads the resulting `<ts>_<uuid>.jsonl` header to persist `Run.metadataJson.session_id`, and `buildResumeCommand` passes `--session <id>` so the next run continues the same conversation. Wider RPC vocabulary is collapsed into the SPEC §14.1 taxonomy (see §14.1 footnote).
