		...
		try {
			const proc = Bun.spawn(
				[
					"bun",
					"run",
					resolve(SCRIPT_DIR, "check-coverage.ts"),
					"--budget",
					budgetPath,
					"--parse",
					path,
...
describe("template budget JSON", () => {
	test("coverage-budgets.json is well-formed", () => {
		const raw = readFileSync(resolve(SCRIPT_DIR, "coverage-budgets.json"), "utf8");
		const budgets = loadBudgets(raw, "coverage-budgets.json");
